   ...
   if(snake[0][0]==foodx&&snake[0][1]==foody)
    ...
    for(int i=1;i<=foodcount;i++)
    {
        ...
        if(snake[0][0]==snake[i][0]&&snake[0][1]==snake[i][1])
        {
            ...
            timer->stop();
            if(QMessageBox::question(this,"waring","Game over,Once a gaain?",QMessageBox::Yes,QMessageBox::No)==QMessageBox::Yes)
               {
                QTime t;
                int i;
                t= QTime::currentTime();
    ...
    if(snake[0][0]<0||snake[0][0]>=COL||snake[0][1]<0||snake[0][1]>=ROW)
    {
        ...
        memcpy(snake,snake1,sizeof(snake));
       timer->stop();
       if(QMessageBox::question(this,"tip","The game is over,Once again?",QMessageBox::Yes,QMessageBox::No)==QMessageBox::Yes)
          {
           QTime t;
           int i;
           t= QTime::currentTime();
